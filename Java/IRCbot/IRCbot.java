        ...
        while (running) {
            try {
                while ((buffer = in.readLine()) != null) {
                    ...
                    respond(buffer, channelList.get(0), ">:)", "Y U EVIL?");
                    respond(buffer, channelList.get(0), "nume?", "Eh?");
                    respond(buffer, channelList.get(0), "ain't nobody got time", "No time? Bull$#it.");
                    respond(buffer, channelList.get(0), "let the betatest begin", "Indeed.");
                    respond(buffer, channelList.get(0), "da fug?", "* Toccata a fuga - J. S. Bach");

                    Grammar.checkGrammar(channelList.get(0));

                    if (buffer.startsWith("PING")) {
                        out.write("PONG :" + buffer.substring(6).getBytes());
                    }
                }
            } catch (IOException e) {
                e.printStackTrace();
